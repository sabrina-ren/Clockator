//
//  KeyConstants.h
//  Clockator
//
//  Created by Sabrina Ren on 1/2/2014.
//  Copyright (c) 2014 Sabrina Ren. All rights reserved.
//

#pragma mark - PFUser
extern NSString *const CKUserObjectId;
extern NSString *const CKUserDisplayNameKey;
extern NSString *const CKUserFriendsKey;
extern NSString *const CKUserLocationKey;
extern NSString *const CKUserIconKey;
extern NSString *const CKUserProfileKey;
extern NSString *const CKUserUpdateDateKey;

#pragma  mark - PFObject FriendRequest class
extern NSString *const CKFriendReqClass;
extern NSString *const CKFriendReqFromUserKey;
extern NSString *const CKFriendReqToUserKey;
extern NSString *const CKFriendReqStatusKey;

#pragma mark - Remote Notifications
extern NSString *const CKUserPrivateChannelKey;
extern NSString *const CKInstallationChannelsKey;
extern NSString *const CKInstallationUserKey;
extern NSString *const CKAPNSAlertKey;
extern NSString *const CKPayloadTypeKey;
extern NSString *const CKPayloadFromUserKey;